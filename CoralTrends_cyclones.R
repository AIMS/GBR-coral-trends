...
cyclones = all.reefs  %>% left_join(cyclones %>% dplyr:::select(-A_SECTOR,-SHELF)) %>%
...
#cyclones$Location <- factor(cyclones$Zone, levels=c('All',"Far Northern Management Area","Cairns/Cooktown Management Area","Townsville/Whitsunday Management Area", "Mackay/Capricorn Management Area"),
#                                labels=c('Great Barrier Reef',"Far Northern","Cairns/Cooktown","Townsville/Whitsunday", "Mackay/Capricorn"))
cyclones$Location <- factor(cyclones$Zone, levels=c("Northern","Central","Southern"),
                                labels=c("Northern","Central","Southern"))

## Fill in the gaps
#cyclones.lookup = expand.grid(Location=c('Great Barrier Reef',"Far Northern","Cairns/Cooktown","Townsville/Whitsunday", "Mackay/Capricorn"),
#                              REPORT_YEAR=seq.int(1985,2017,by=1))
cyclones.lookup = expand.grid(Location=c("Northern","Central","Southern"),
...
cyclones = cyclones %>% mutate(Zone=ifelse(Location=='Northern', 'Northern',
                                    ifelse(Location=='Central', 'Central',
                                    ifelse(Location=='Southern', 'Southern', 'All'))))
...
cyclones.sum.all$Location <- factor(cyclones.sum.all$Zone, levels=c('All',"Northern","Central","Southern"),
                                labels=c('Great Barrier Reef',"Northern","Central","Southern"))

## Fill in the gaps
#cyclones.lookup = expand.grid(Location=c('Great Barrier Reef',"Far Northern","Cairns/Cooktown","Townsville/Whitsunday", "Mackay/Capricorn"),
#                                REPORT_YEAR=seq.int(1985,2017,by=1))
#cyclones.sum.all = cyclones.sum.all %>% right_join(cyclones.lookup)                                
